...
- 执行上下文包含三个部分：
  - 变量环境（LexicalEnvironment）：指一个**词法环境**对象，用于存储该执行上下文内`var`变量声明绑定和函数声明绑定等
  - 词法环境（VariableEnvironment）：指一个**词法环境**对象，用于存储该执行上下文内`let`和`const`变量声明绑定
...
- 代码执行过程：
  - 编译阶段
    - 进行**变量提升**，变量和函数声明存储到**变量环境**中，变量绑定值设置为`undefine`，函数绑定值设置为该函数执行代码
  - 执行阶段
    - 执行当前激活执行上下文中的**可执行代码**，完成对变量环境内的变量值的分配和访问

## 词法环境

## 作用域

作用域负责收集和维护由所有声明的标识符（变量）组成的一系列查询，并实施一套严格的规则，确定当前执行的代码对这些标识符的访问权限。

作用域分为两种：**静态作用域**（词法作用域）和**动态作用域**

**词法作用域**的确定发生在代码编译阶段

JavaScript 采用的是词法作用域，因此函数的作用域在函数定义的时候就决定了

> ES5只有全局作用域和函数作用域，ES6新增了块级块级作用域，JS引擎将块级作用域变量声明存储到当前执行上下文的词法环境组件（VariableEnvironment）中， 并在声明变量之前创建暂时性死区
